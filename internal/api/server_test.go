...
func TestModelsWithClientVersionReturnsCodexCatalog(t *testing.T) {
	...
	// The global registry is shared across tests, so the alphabetical rank of a
	// custom model can vary. Verify the priority is above the template maximum
	// and remains aligned to the 100-point custom-model priority buckets.
	maxTemplatePriority := codexClientTestMaxTemplatePriority(t)
	if got := int(codexClientTestPriority(custom["priority"])); got < maxTemplatePriority+100 || (got-maxTemplatePriority)%100 != 0 {
		t.Fatalf("custom priority = %v, want >= %d and aligned to 100-point buckets", custom["priority"], maxTemplatePriority+100)
	}
