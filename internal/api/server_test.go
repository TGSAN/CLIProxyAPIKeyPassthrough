...
func TestModelsWithClientVersionReturnsCodexCatalog(t *testing.T) {
	...
	// Custom model priority = maxTemplateModelPriority + 100*(alphabetical rank+1).
	// The global registry is shared across tests so rank may vary; verify that the
	// priority is above the maximum template priority (43 with the current JSON) and
	// is a correct multiple (i.e. priority ≡ 43 mod 100).
	if got := int(codexClientTestPriority(custom["priority"])); got < 143 || (got-43)%100 != 0 {
		t.Fatalf("custom priority = %v, want >= 143 and (priority-43) divisible by 100", custom["priority"])
	}
