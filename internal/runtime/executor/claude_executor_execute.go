...
func (e *ClaudeExecutor) Execute(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	if claudeThinkingReplayEnabled(ctx, auth, req, opts) {
	...
	trimmedResponse := bytes.TrimSpace(data)
	if upstreamStream && (bytes.HasPrefix(trimmedResponse, []byte("data:")) || bytes.HasPrefix(trimmedResponse, []byte("event:"))) {
	...
	if errorMessage := strings.TrimSpace(gjson.GetBytes(out, "error.message").String()); errorMessage != "" {
		return resp, statusErr{code: http.StatusBadGateway, msg: errorMessage}
	}
